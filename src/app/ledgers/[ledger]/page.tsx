...
import { JournalTable } from "./LedgerContents";
import { useEffect, useState } from "react";
import { loadTrialBalance } from "@/services/loadTrialBalance";
    ...
    useEffect(() => {
      const fetchJournals = async () => {
        try {
          ...
          const loadedTrialBalance = await loadTrialBalance(owner);
          console.log("Loaded Journal:", loadedTrialBalance.trialBalance.filter((item: { _id: any; })=> item._id === ledgerlId));
          setSelectedTrialBalance(loadedTrialBalance.trialBalance.filter((item: { _id: any; })=> item._id === ledgerlId)[0]);
          setIsLoading(false);
        } catch (error) {
          ...
          {isLoading ? (
            ...
            <div className="flex flex-1 items-center justify-center rounded-lg border border-dashed shadow-sm">
              <div className="items-center gap-1 text-center">
                <JournalTable trialbalance={selectedTrialBalance} />
              </div>
            </div>
          )}
        </main>
